...
**Note:**<br>
If you have used **Stage 1** to train the model, you may set the `freeze_encoder` in `configs/cls.yaml` as **False** at the **Stage 2**.<br>
If not, set the `freeze_encoder` in `configs/cls.yaml` as **True** at the **Stage 2**.
...
Just train the model at **Stage 2** with setting the `freeze_encoder` in `configs/cls.yaml` as **True**, if you want to replicate our result.
